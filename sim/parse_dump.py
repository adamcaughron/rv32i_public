...
def parse_riscv_elfdump(elfdump_file):
  ...
  with open(elfdump_file, "r") as f:
          ...
          try:
              test_entry_address = int(re.match(r"\s*([0-9A-Fa-f]+):\s+([0-9A-Fa-f]{8})", lines[1]).group(1)[1:], 16)
          ...
          jump_to_test = f"\tmem[32'h{f"{0x7c>>2:08x}"}] = 32'h{f"{jump_to_test_instr:08x}"};\n"
